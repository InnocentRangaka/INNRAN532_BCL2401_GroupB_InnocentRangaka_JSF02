<script>
    ...
    const isInWishList = (id) => {
      // implement logic to check if product is in wishlist
    };
    const addToFavourites = (id) => {
      // implement logic to add product to favourites
    };
    let cart = {
      add: (product, el) => {
    ...
    let ratingStars = (rating) => {
      // implement logic to generate rating stars
    };
          ...
          <div class="flex-1">
            ...
            {#if product.rating}
              <div class="flex items-left -ml-2 mb-2">
                {@html ratingStars(product.rating.rate)}
              </div>
            {/if}
          ...
          <div class="flex mt-1 space-x-2 place-items-center">
            ...
            <div class="flex justify-end space-x-2 place-items-center items-center">
              <button
                ...
                on:click={() => addToFavourites(product.id)}
              >
                <svg
